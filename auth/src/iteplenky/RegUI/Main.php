...
namespace iteplenky\RegUI;
    ...
    class Main extends PluginBase
    {
        ...
        public function onEnable() : void
        {
            ...
            $this->database->exec("CREATE TABLE IF NOT EXISTS players(player TEXT PRIMARY KEY COLLATE NOCASE, password TEXT, IpAddress TEXT, vkId TEXT)"); //создание таблицы в базе данных
        ...
        {
            $db = $this->database->prepare("SELECT player FROM players WHERE player =:player;");
        ...
        {
            $db = $this->database->prepare("INSERT OR REPLACE INTO players(player, password, IpAddress, vkId) VALUES(:player, :password, :IpAddress, :vkId)");
        ...
        {
            $db = $this->database->prepare("SELECT * FROM players WHERE player=:player;");
        ...
        {
            $db = $this->database->prepare("UPDATE players SET password=:newpassword WHERE player=:player;");
        ...
        {
            $db = $this->database->prepare("DELETE FROM players WHERE player=:player;");
            $db->bindValue(":player", strtolower($player));
            $db->execute();
            $db->close();
            
            if ($this->isLogined($player))
                $this->setLogined($player, false);
        ...
        {
            $db = $this->database->prepare("UPDATE players SET IpAddress=:IpAddress WHERE player=:player;");
        ...
        {
            $rows = $this->database->query("SELECT COUNT(*) as count FROM players");
